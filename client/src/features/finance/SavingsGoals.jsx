...
const SavingsGoals = ({ savingsGoals, currency }) => {
  ...
  // This function opens the modal to create a NEW goal
  const openAddGoalModal = () => showModal(<SavingsGoalForm />, { title: 'הוספת יעד חדש' });
  
  // This function opens the modal to add funds to an EXISTING goal
  const openAddFundsModal = (goal) => {
    // We pass the specific 'goal' object to the form
      ...
      <div id="savings-goals-container" className="savings-goals-grid">
        {savingsGoals && savingsGoals.length > 0 ? (
          savingsGoals.map(goal => {
            // --- All logic using 'goal' must be INSIDE this map function ---
